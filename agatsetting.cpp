...
//    model->item(tSensorType::UnknownType)->setEnabled(false);
    ...
    leData->setFixedSize(QSize(200, 20));
    leData->setText("");
...
void cAgatSetting::setData(QByteArray &data)
{
    ...
    if (data.length() == 5) //КРУ-1
    {
        ...
        _values[0] = "0x" + QString::number(data[0], 16);   // Это статус
        agatChannel param;
    ...
    else if (data.length() == 14) //Вибро-1
    {
        ...
        _values[0] = "0x" + QString::number(data[0], 16);   // Это байт ошибки
        _values[1] = "0x" + QString::number(data[1], 16);   // Это байт статуса
        agatChannel param;
    ...
    else if (data.length() == 15) //Коралл-1 или Коралл+
    {
        ...
        _values[3] = "0x" + QString::number(data[12], 16);   // Это байт ошибки
        _values[4] = "0x" + QString::number(data[13], 16);   // Это байт статуса
        _values[5] = "0x" + QString::number(data[14], 16);   // резерв
    }
    else if (data.length() == 27) //Коралл+
    {
        ...
        _values[6] = "0x" + QString::number(data[24], 16);   // Это байт ошибки
        _values[7] = "0x" + QString::number(data[25], 16);   // Это байт статуса
        _values[8] = "0x" + QString::number(data[26], 16);   // резерв
    }
    else
    {
        if(_type != UnknownType) setType(UnknownType);
    };
    updateValues();
}
